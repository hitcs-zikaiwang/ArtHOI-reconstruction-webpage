...
var display_level = 2;
var video_names = ["result-video-1", "result-video-2", "result-video-3", "result-video-4", "result-video-5", "result-video-6"];
...
function load_videos() {
  ...
  for (var i = 0; i < video_names.length; i++) {
    var el = document.getElementById(video_names[i]);
...
$(() => {
  ...
  setInterval(() => {
    $(".image-compare").each((_index, parent) => {
      ...
      if (videos.length == 0 || !videos[current_video_idx]) return;
...
function set_play_pause_icon() {
  ...
  if (!button || !current_video) return;
